...
import { CierreCohorteView } from '@/components/ui/cierre-cohorte-view'; 
...
export default async function SolicitudDetallePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ✨ FIX CRÍTICO: Atrapamos el ID real venga como venga en el payload del Token
  const realCurrentUserId = currentUser?.sub || currentUser?.id || currentUser?.userID;
  ...
  const getStatusColor = (estado: string) => {
    const colors: Record<string, string> = { 'pendiente': 'orange', 'aprobada': 'green', 'rechazada': 'red', 'under_review': 'orange' };
    ...
    <Container maxW="container.xl" py={10}>
      <VStack align="stretch" spacing={8}>
        ...
        <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6}>
          ...
          <StatCard label="Fecha de Registro" value={new Date(solicitud.fecha_creacion).toLocaleDateString()} color="blue.500" />
        ...
        <Box bg="gray.50" _dark={{ bg: "gray.900", borderColor: "gray.700" }} p={6} rounded="xl" border="1px dashed" borderColor="gray.300">
          ...
          <AdminActions 
            ...
            // ✨ FIX CRÍTICO: Forzamos que se envíe como String absoluto
            currentUserId={String(realCurrentUserId)} 
          />
        </Box>
